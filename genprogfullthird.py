...
def diffMats(img_mat, loop_mat):
	...
	return (1.5*correct_connected_diff - shouldnt_be_connected_diff, correct_connected_diff, shouldnt_be_connected_diff)
...
def toProg(for_tup_arr, fname, img):
	...
	for for_tup in for_tup_arr:
		...
		if len(all_copies) > 0:
			...
			tups.append((for_tup, resize(img1, (17,17,3))))
		else:
			continue	
	f = open(fname, "w+")
	print len(tups)
...
def toMaskArr(for_tup_arr, fname, img):

	img2 = np.full((img.shape[0], img.shape[1]), 0, dtype=np.uint8)
	...
	for for_tup in for_tup_arr:
		...
		if len(all_copies) > 0:
			#imsave("testcopies" + str(random.randint(0,100)) + ".png", np.concatenate(all_copies))
			#print(all_copies)
			all_copies = [(i, sum(map(lambda k: emd_samples(i[2], k[2], bins = 100), all_copies))) for i in all_copies]
			all_copies.sort(key = lambda i: i[1])
			best = all_copies[0][0]
			img1 = copy.copy(img[H[best[0]]:H[min(len(H) - 1, best[0] + for_tup.i_size)], W[best[1]]:W[min(len(W) - 1, best[1] + for_tup.j_size)]])
			sub_height = img1.shape[0]
			...
			for i in range(for_tup.i_n):
				for j in range(for_tup.j_n):
						try:
							y = H[i*for_tup.i_mul + for_tup.i_offset]
							x = W[j*for_tup.j_mul + for_tup.j_offset]
							img2[y:y+sub_height, x:x+sub_width] = 255
						except: pass
	print(np.count_nonzero(img2))
        imsave(fname, img2)

def toPngWithAlpha(q,z):
	start = np.zeros((img_size,img_size,4)).astype("uint8")
	start[:,:,0:3] = img_as_ubyte(imread("approx-" + str(img_size) + "-" + str(q) + "/" + str(z) + "-approx.png"))
...
for z_ in range(1):#len(image_mats)):	
						...
						if str(f) not in seen:
							...
							if score > 0:
								while True:
									...
									new_fs = [(copy.copy(f), True), (copy.copy(f), False)]
									new_fs[0][0].i_n += 1
									new_fs[1][0].j_n += 1
									if f.i_size < 3 and f.i_n > 1 and f.i_size < f.i_mul - 1:
										f_ = copy.copy(f)
										f_.i_size += 1
										new_fs.append((f_, True))
									if f.j_size < 3 and f.j_n > 1 and f.j_size < f.j_mul - 1:
										f_ = copy.copy(f)
										f_.j_size += 1
										new_fs.append((f_,True))				
									...
									mat_scores = [diffMats(img_mat, matFromForTup(k[0], img_mat))[0] for k in new_fs]
									print(mat_scores)
									#print("mat scores: " + str(mat_scores))
									#mat_sccores = [mat_scores[k]+1 if new_fs[k][1] else mat_scores[k] for k in range(len(new_fs))]

									if max(mat_scores) <= score:
										break
									else:
										f = max([(new_fs[k], mat_scores[k]) for k in range(len(new_fs))], key = lambda q: q[1])[0][0]
										score = max(mat_scores)
										print((f.i_size, f.j_size))
	...
	good_for_tups.sort(key = lambda i: getCoverage(i[0]), reverse=True)# reverse=True)
	...
	for for_tup in good_for_tups:
		...
		pairs = [k for k in list(itertools.product(i_s, j_s)) if k not in inclusion]
		print(pairs)
		#print "len pairs: " + str(len(pairs)) + " getCoverage " + str(getCoverage(for_tup))
		length = len([x for x in pairs])
		if length > 0:
			#if len([x for x in pairs if x in inclusion]) < 20:
				for pair in pairs:
					inclusion.add(pair)
				full_set.append((for_tup, sum(map(len, pairs))))
	print "full-set: " + str(len(full_set))
	
	full_set.sort(key = lambda k: k[1], reverse=True)
	full_set = [k[0] for k in full_set]
	full_set = full_set[:12]
